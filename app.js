...
function updateDashboard() {
    ...
    // Enhanced status with financial health
    const net = Money.subtract(inc, exp);
    const budgetStatus = document.getElementById('budget-status');
    const healthScore = document.getElementById('health-score');
    
    if (budgetStatus) {
        if (net >= inc * 0.1) { // More than 10% positive
            budgetStatus.className = 'status-banner status-excellent';
            budgetStatus.innerHTML = `🎉 Excellent! ₱${Math.abs(net).toLocaleString()} under budget.`;
            healthScore.className = 'health-score score-excellent';
            healthScore.textContent = '⭐ Excellent';
        } else if (net >= 0) {
            budgetStatus.className = 'status-banner status-positive';
            budgetStatus.innerHTML = `👍 Good! ₱${Math.abs(net).toLocaleString()} under budget.`;
            healthScore.className = 'health-score score-good';
            healthScore.textContent = '👍 Good';
        } else if (net >= -inc * 0.1) { // Less than 10% negative
            budgetStatus.className = 'status-banner status-warning';
            budgetStatus.innerHTML = `⚠️ Warning! ₱${Math.abs(net).toLocaleString()} over budget.`;
            healthScore.className = 'health-score score-warning';
            healthScore.textContent = '⚠️ Needs Care';
        } else {
            budgetStatus.className = 'status-banner status-critical';
            budgetStatus.innerHTML = `
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <strong>🚨 Budget Alert!</strong> 
                        You're <strong>₱${Math.abs(net).toLocaleString()} over budget</strong>.
                        <small style="display:block; margin-top:4px; opacity:0.8;">
                            Consider reviewing your spending patterns.
                        </small>
                    </div>
                    <button class="btn-small" onclick="showSection('transactions')" style="background: white; color: #dc2626;">
                        Fix Now
                    </button>
                </div>
            `;
            healthScore.className = 'health-score score-critical';
    ...
    if (chartTotal) chartTotal.textContent = Formatter.currency(exp);
    
    drawChart(cats, exp); 
    checkAlerts(cats);
}

// ENHANCED ALERTS WITH INSIGHTS
function checkAlerts(expenses) {
    const alertsList = document.getElementById('alerts-list');
    if (!alertsList) return;
    
    const totalExpenses = Object.values(expenses).reduce((a, b) => Money.add(a, b), 0);
    let alertsHTML = '';
    ...
    Object.entries(expenses).forEach(([category, amount]) => {
        ...
        if (percentage > 50) {
            alertType = 'high';
            message = `Consider reducing ${category.toLowerCase()} expenses`;
        } else if (percentage > 30) {
            alertType = 'medium';
            message = `${category} spending is moderate`;
        } else {
            alertType = 'low';
            message = `${category} spending is on track`;
        }
        
        const colors = { high: '#dc2626', medium: '#f59e0b', low: '#3b82f6' };
        
        alertsHTML += `
            <li style="border-left-color: ${colors[alertType]};">
                <strong>${getCategoryEmoji(category)} ${category} ${alertType === 'high' ? 'High' : 'Spending'}</strong>
                <span>${Formatter.currency(amount)} (${percentage.toFixed(1)}%)</span>
                ${message ? `<small style="display:block; margin-top:4px; color: var(--text-muted);">${message}</small>` : ''}
            </li>
        `;
    });
    
    // Add cash flow alert if expenses > income
    const income = parseFloat(document.getElementById('total-income').textContent.replace(/[^\d.]/g, ''));
    const expense = parseFloat(document.getElementById('total-expense').textContent.replace(/[^\d.]/g, ''));
    
    if (expense > income) {
        alertsHTML += `
            <li style="border-left-color: #dc2626;">
                <strong>📉 Negative Cash Flow</strong>
                <span>-${Formatter.currency(expense - income)} this month</span>
    ...
    alertsList.innerHTML = alertsHTML || '<li>No alerts this month. Great job! 🎉</li>';
}
